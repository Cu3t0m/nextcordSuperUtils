...
def generate_embeds(
    ...
    timestamp: datetime = nextcord.Embed.Empty,
...
class ButtonsPageManager:
    ...
    async def run(self):
        ...
        from nextcord_components import ActionRow, Button, ButtonStyle, nextcordComponents

        nextcordComponents(self.ctx.bot)
        ...
        # message_to_send must be of type embed, sadly, nextcord_components breaks the Messageable.send method
...
class PageManager:
    ...
    async def run(self):
        ...
        message_to_send = self.messages[self.index]
        if isinstance(message_to_send, nextcord.Embed):
