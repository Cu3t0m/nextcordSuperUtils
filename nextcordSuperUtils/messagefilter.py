...
class MessageResponseGenerator(ABC):
    ...
    @abstractmethod
    def generate(self, message: nextcord.Message) -> Union[bool, Any]:
        """
...
class DefaultMessageResponseGenerator(MessageResponseGenerator):
    ...
    nextcord_INVITE_RE = re.compile(
        r"(?:(?:http|https)://)?(?:www.)?(?:disco|nextcord|nextcordapp).("
    ...
    def generate(self, message: nextcord.Message) -> Union[bool, Any]:
        ...
        return self.URL_RE.match(message.content) or self.nextcord_INVITE_RE.match(
            message.content
        )
...
class MessageFilter(EventManager, CacheBased):
    """
    Represents a nextcordSuperUtils message filter that filters messages and finds inappropriate content.
    """
