...
class PrefixManager(DatabaseChecker):
    ...
    async def get_prefix(self, guild: Union[nextcord.Guild, Any]) -> Iterable[str]:
        """
        |coro|

        Returns the prefix or the guild or the default prefixes.

        :param nextcord.Guild guild: The guild.
        :return: The prefixes.
        :rtype: Iterable[str]
        """
    ...
    @DatabaseChecker.uses_database
    async def delete_prefix(self, guild: nextcord.Guild) -> None:
        """
        |coro|
    ...
    @DatabaseChecker.uses_database
    async def set_prefix(self, guild: nextcord.Guild, prefix: str) -> None:
        """
        |coro|
    ...
    async def __get_prefix(self, bot, message: nextcord.Message) -> Tuple[str]:
