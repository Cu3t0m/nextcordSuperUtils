...
class UnbanFailure(Exception):
    """Raises an exception when the user tries to unban a nextcord.User without passing the guild."""
...
class BanManager(DatabaseChecker, Punisher):
    ...
    @staticmethod
    async def get_ban(
        member: Union[nextcord.Member, nextcord.User], guild: nextcord.Guild
    ...
    @DatabaseChecker.uses_database
    async def unban(
        self, member: Union[nextcord.Member, nextcord.User], guild: nextcord.Guild = None
    ) -> bool:
        """
        |coro|
        ...
        :param Union[nextcord.Member, nextcord.User] member: The member or user to unban.
        :param nextcord.Guild guild: The guild to unban the member from.
        :return: A bool representing if the unban was successful.
        :rtype: bool
        :raises: UnbanFailure: Cannot unban a nextcord.User without a guild.
        """

        if isinstance(member, nextcord.User) and not guild:
            raise UnbanFailure("Cannot unban a nextcord.User without a guild.")
    ...
    async def __handle_unban(
        self, time_of_ban: Union[int, float], member: nextcord.Member, reason: str
    ) -> None:
        """
        |coro|
        ...
        :param nextcord.Member member: The member to unban.
