...
class BirthdayManager(DatabaseChecker):
    ...
    async def create_birthday(
        self, member: nextcord.Member, member_birthday: float, timezone: str = "UTC"
    ) -> None:
        """
        |coro|
    ...
    @DatabaseChecker.uses_database
    async def get_birthday(self, member: nextcord.Member) -> Optional[BirthdayMember]:
        """
        |coro|
    ...
    @DatabaseChecker.uses_database
    async def get_upcoming(self, guild: nextcord.Guild) -> List[BirthdayMember]:
        """
        |coro|
