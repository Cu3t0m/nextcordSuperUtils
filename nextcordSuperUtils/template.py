...
class TemplateInfo(DictionaryConvertible):
    ...
    def from_dict(cls, convert_from: Dict[str, Any]) -> TemplateInfo:
        ...
        convert_from["explict_content_filter"] = nextcord.ContentFilter(
...
class TemplateTextChannel(DictionaryConvertible):
    ...
    def __init__(
        ...
        nsfw: bool,
        channel_id: int,
        overwrites: Dict[int, nextcord.PermissionOverwrite],
    ):
        self.name = name
        self.position = position
...
class TemplateRole(DictionaryConvertible):
    ...
    def __init__(
        ...
        self.permissions = nextcord.Permissions(permissions)
...
class Template:
    ...
    async def apply_settings(
        ...
        reason: str,
        channels: Dict[int, Union[nextcord.VoiceChannel, nextcord.TextChannel]],
    ) -> None:
        await guild.edit(
    ...
    async def apply_roles(
        self, guild: nextcord.Guild, reason: str
    ) -> Dict[int, nextcord.Role]:
    ...
    async def apply_categories(
        self, guild: nextcord.Guild, reason: str
    ) -> Dict[int, nextcord.CategoryChannel]:
    ...
    async def apply_channels(
        ...
        guild: nextcord.Guild,
        reason: str,
        categories: Dict[int, nextcord.CategoryChannel],
        roles: Dict[int, nextcord.Role],
    ) -> Dict[int, nextcord.TextChannel]:
    ...
    async def apply_voice_channels(
        ...
        categories: Dict[int, nextcord.CategoryChannel],
        roles: Dict[int, nextcord.Role],
    ) -> Dict[int, nextcord.VoiceChannel]:
    ...
    async def apply(self, guild: nextcord.Guild) -> None:
    ...
    def get_overwrite(
    ...
    ) -> Dict[int, nextcord.PermissionOverwrite]:
        result_overwrites = {}
        ...
        for overwrite in overwrites:
            if overwrite["overwrite_object"] == overwrite_object:
                ...
                ] = nextcord.PermissionOverwrite.from_pair(
                    nextcord.Permissions(overwrite["overwrite_pair"]),
                    nextcord.Permissions(overwrite["overwrite_second_pair"]),
                )
...
class TemplateManager(DatabaseChecker):
    ...
    async def get_templates(self, guild: nextcord.Guild = None) -> List[Template]:
    ...
    async def create_template(self, guild: nextcord.Guild) -> Template:
        self._check_database()
