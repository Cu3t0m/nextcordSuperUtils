...
TwitchManager = nextcordSuperUtils.TwitchManager(bot, "CLIENT_ID", "CLIENT_SECRET")


def add_stream_fields(embed: nextcord.Embed, stream: dict):
    embed.add_field(
        name="Title",
...
@TwitchManager.event()
async def on_stream(guild: nextcord.Guild, streams: List[dict]):
...
@TwitchManager.event()
async def on_stream_end(guild: nextcord.Guild, streams: List[dict]):
    channel = guild.get_channel(...)
...
async def lookup(ctx, *, channel: str):
    ...
    embed = nextcord.Embed(title=f"'{channel}' is currently streaming!", color=0x00FF00)
