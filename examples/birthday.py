...
BirthdayManager = nextcordSuperUtils.BirthdayManager(bot)
...
@bot.command()
async def birthday(ctx, member: nextcord.Member = None):
    ...
    embed = nextcord.Embed(title=f"{member}'s Birthday", color=0x00FF00)
...
async def delete_birthday(ctx):
    ...
    embed = nextcord.Embed(title=f"Deleted {ctx.author}'s Birthday.", color=0x00FF00)
...
async def setup_birthday(ctx):
    ...
    answers, timed_out = await nextcordSuperUtils.questionnaire(
