...
class Leveling(commands.Cog, nextcordSuperUtils.CogManager.Cog):
    def __init__(self, bot):
        self.bot = bot

        self.LevelingManager = nextcordSuperUtils.LevelingManager(bot, award_role=True)
    ...
    @nextcordSuperUtils.CogManager.event(nextcordSuperUtils.LevelingManager)
    ...
    async def rank(self, ctx, member: nextcord.Member = None):
        ...
        image = await self.ImageManager.create_leveling_profile(
            ...
            background=nextcordSuperUtils.Backgrounds.GALAXY,
    ...
    @commands.command()
    async def set_roles(self, ctx, interval: int, *roles: nextcord.Role):
