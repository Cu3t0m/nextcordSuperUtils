...
def get_user_spotify(member: nextcord.Member) -> Optional[nextcord.Spotify]:
    """
    Returns the member's spotify activity, if applicable
    :param nextcord.Member member: The member.
    :return: The member's spotify activity.
    :rtype: Optional[nextcord.Spotify]
    """
...
class Music(commands.Cog, nextcordSuperUtils.CogManager.Cog, name="Music"):
    def __init__(self, bot):
        ...
        self.ImageManager = nextcordSuperUtils.ImageManager()
        super().__init__()
    ...
    async def on_music_error(self, ctx, error):
        errors = {
            nextcordSuperUtils.NotPlaying: "Not playing any music right now...",
            nextcordSuperUtils.NotConnected: f"Bot not connected to a voice channel!",
            nextcordSuperUtils.NotPaused: "Player is not paused!",
            nextcordSuperUtils.QueueEmpty: "The queue is empty!",
            nextcordSuperUtils.AlreadyConnected: "Already connected to voice channel!",
            nextcordSuperUtils.RemoveIndexInvalid: "Remove index is invalid!",
            nextcordSuperUtils.SkipError: "There is no song to skip to!",
            nextcordSuperUtils.UserNotConnected: "User is not connected to a voice channel!",
            nextcordSuperUtils.InvalidSkipIndex: "That skip index is invalid!",
        }
    ...
    async def on_ready(self):
        database = nextcordSuperUtils.DatabaseManager.connect(...)  # Database connection
    ...
    async def now_playing(self, ctx):
        if player := await self.MusicManager.now_playing(ctx):
            ...
            embed = nextcord.Embed(
                ...
                color=nextcord.Color.from_rgb(0, 255, 255),
            )
            embed.add_field(
                name="Played",
    ...
    async def history(self, ctx, songs_per_page: int = 15):
        if queue := (await self.MusicManager.get_queue(ctx)):
            ...
            await nextcordSuperUtils.PageManager(ctx, embeds, public=True).run()
    ...
    @commands.command()
    async def spotify_user_song(self, ctx, member: nextcord.Member = None):
    ...
    @commands.command()
    @ensure_voice_state()
    async def play_user_spotify(self, ctx, member: nextcord.Member = None):
        member = member if member else ctx.author
        spotify_result = get_user_spotify(member)
    ...
    async def playlists(self, ctx, user: nextcord.user = None):
    ...
    @playlists.command()
    @ensure_voice_state()
    async def play(self, ctx, index: int, user: nextcord.member = None):
